...
float* CSegy::ReadTraceData(int nIndex)
{
    ...
    char *psz = new char[m_nSampleNum*m_nDataLenth];
    float *p = new float[m_nSampleNum];
    fread(psz,sizeof(char),m_nSampleNum*m_nDataLenth,m_fpr);


    for (int i=0;i<m_nSampleNum;i++)
    {
        memcpy((char*)p+i*sizeof(float),psz+i*m_nDataLenth,m_nDataLenth);
        if (m_bDataIBMFmt)
            ibm2ieee(&p[i],m_bDataWS);
...
float* CSegy::GetTraceData(int nIndex )
{
    ...
    float *pdata = new float[m_nSampleNum];
